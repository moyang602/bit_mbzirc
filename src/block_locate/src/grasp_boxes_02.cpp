...
void action()
{
  ...
  HTuple  hv_Pose, hv_Score, hv_SurfaceMatchingResult, hv_Length;
  HTuple  hv_transX, hv_transY, hv_transZ, hv_rotX, hv_rotY;
  HTuple  hv_rotZ, hv_order, hv_ValueOfPose, hv_OrderOfTransform;
  HTuple  hv_OrderOfRotation, hv_ViewOfTransform, hv_Diameter;
  HTuple  hv_ObjectModel3DEdges, hv_ObjectModel3DRigidTrans;

  //The object of this example is to classify different
  //kinds of wood according to their surface texture.
  FileExists("classify_wood_boxes.gmc", &hv_FileExists);
  ...
  ReadClassMlp("classify_wood_boxes.gmc", &hv_MLPHandle);
  ...
  //读取zed采集后的图像
  //判断场景的颜色
  hv_ReadPath = "/home/srt/Project/box_find_project/find_box/zed_test.jpg";
  ReadImage(&ho_Image, hv_ReadPath);
  gen_features(ho_Image, &hv_FeatureVector);
  ...
  if (0 != (hv_result_color==HTuple("red")))
  {
    ...
    TriangulateObjectModel3d(hv_ObjectModel3DBox, "greedy", HTuple(), HTuple(), &hv_ObjectModel3DModel, 
        &hv_Information);
    hv_FileName = "redbox_edge_supported.sfm";
  ...
  else if (0 != (hv_result_color==HTuple("green")))
  {
    ...
    TriangulateObjectModel3d(hv_ObjectModel3DBox, "greedy", HTuple(), HTuple(), &hv_ObjectModel3DModel, 
        &hv_Information);
    hv_FileName = "greenbox_edge_supported.sfm";
    FileExists(hv_FileName, &hv_FileExists);
    if (0 != (hv_FileExists.TupleNot()))
    {
      if (HDevWindowStack::IsOpen())
  ...
  else if (0 != (hv_result_color==HTuple("blue")))
  {
    ...
    TriangulateObjectModel3d(hv_ObjectModel3DBox, "greedy", HTuple(), HTuple(), &hv_ObjectModel3DModel, 
        &hv_Information);
    hv_FileName = "bluebox_edge_supported.sfm";
    FileExists(hv_FileName, &hv_FileExists);
    if (0 != (hv_FileExists.TupleNot()))
    {
      if (HDevWindowStack::IsOpen())
  ...
  TupleRegexpSelect(hv_ImageFiles, (HTuple("\\.(tif|tiff)$").Append("ignore_case")), 
      &hv_ImageFiles);
  {
  HTuple end_val112 = (hv_ImageFiles.TupleLength())-1;
  ...
  for (hv_Index=0; hv_Index.Continue(end_val112, step_val112); hv_Index += step_val112)
  {
    ...
    //旋转顺序是'gba'方式
    GetPoseType(hv_ValueOfPose, &hv_OrderOfTransform, &hv_OrderOfRotation, &hv_ViewOfTransform);
    //write_pose (ValueOfPose, 'campose.dat')
