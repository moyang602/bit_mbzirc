...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 1 or dataset == 'L_Shelf':
      self.names = L_Shelf_class_name
    elif num_classes == 1 or dataset == 'L_Shelf_HP':
      self.names = L_Shelf_class_name
      self.num_class = 1
      self.num_kps = 6  # 增加关键点的个数定义
      self.colors_hp = [(255, 0, 255), (255, 0, 0), (0, 0, 255),
                        (255, 0, 0), (0, 0, 255), (255, 0, 0)]  #留下6个就够了
  ...
  def add_KPS_hp(self, points, img_id='default'):
    points = np.array(points, dtype=np.int32).reshape(self.num_kps, 2)
  ...
  def show_all_imgs(self, pause=False, time=0):
    if not self.ipynb:
      ...
      if cv2.waitKey(0 if pause else 1) == 27:
        ...
        print('Not ipynb. And show all images')
        self.ax = None
        nImgs = len(self.imgs)
        print('The number of images is ',nImgs)
