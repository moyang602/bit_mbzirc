...
class MultiKPSLoss(torch.nn.Module):
...
class MultiKPSTrainer(BaseTrainer):
    def __init__(self, opt, model, optimizer=None):
        super(MultiKPSTrainer, self).__init__(opt, model, optimizer=optimizer)
    ...
    def debug(self, batch, output, iter_id):
        ...
        dets[:, :, :4] *= opt.down_ratio  #opt.input_res / opt.output_res
        dets[:, :, 5:17] *= opt.down_ratio#opt.input_res / opt.output_res
        print('dets shape[2] is : ',dets.shape[2])
        dets_gt = batch['meta']['gt_det'].numpy().reshape(1, -1, dets.shape[2])
        dets_gt[:, :, :4] *= opt.down_ratio  #opt.input_res / opt.output_res
        dets_gt[:, :, 5:17] *=opt.down_ratio   #opt.input_res / opt.output_res
        for i in range(1):
            debugger = Debugger(
            ...
            for k in range(len(dets[i])):
                if dets[i, k, 4] > opt.center_thresh:
                    ...
                    debugger.add_KPS_hp(dets[i, k, 5:17], img_id='out_pred')
            ...
            for k in range(len(dets_gt[i])):
                if dets_gt[i, k, 4] > opt.center_thresh:
                    ...
                    debugger.add_KPS_hp(dets_gt[i, k, 5:17], img_id='out_gt')
